...
public class Day4 implements Day {
    ...
    public Object getResultP1(String input) {
        List<Event> events = Arrays.stream(input.split("\n"))
                .map(it -> createEvent(it))
                .sorted()
                .collect(Collectors.toList());

        List<Shift> shifts = new ArrayList<>();
        List<Event> shiftList = createEventList(events, shifts);

        if (!shiftList.isEmpty()) shifts.add(createShift(shiftList));

        Long sleepyGuard = getSleepiestGuard(shifts);

        int sleepiestMinute = getSleepiestMinute(shifts, sleepyGuard);
        return sleepiestMinute * sleepyGuard;
    }

    private List<Event> createEventList(List<Event> events, List<Shift> shifts) {
        List<Event> shiftList = null;
        for (Event e : events) {
            if (e.getType().equals(EventType.GUARD)) {
                if (shiftList != null) shifts.add(createShift(shiftList));
    ...
    public Object getResultP2(String input) {
        ...
        Long consistentGuard = getConsistentGuard(shifts);

        int min = getSleepiestMinute(shifts, consistentGuard);

        return consistentGuard * min;
    }

    private Long getConsistentGuard(List<Shift> shifts) {
        Set<Long> guards = new HashSet<>();
        for (var shift: shifts) {
            guards.add(shift.getGuardId());
        }

        Map<Long, Integer> favoriteMinute = new HashMap<>();
        for (var guard: guards) {
            int[] mins = new int[60];
            for (var shift: shifts.stream().filter(it -> it.getGuardId().equals(guard)).collect(Collectors.toList())) {
                for (int min: shift.getSleepingMinutes()) {
                    mins[min]++;
                }
            }
        ...
        for (Long guard: guards) {
            if (favoriteMinute.get(guard) > max) {
                max = favoriteMinute.get(guard);
                maxGuard = guard;
            }
        }
        return maxGuard;
    ...
    private int getSleepiestMinute(List<Shift> shifts, Long sleepyGuard) {
        int[] mins = new int[60];
        for (var shift: shifts.stream().filter(it -> it.getGuardId().equals(sleepyGuard)).collect(Collectors.toList())) {
            for (int min: shift.getSleepingMinutes()) {
                mins[min]++;
            }
        }
    ...
    private Long getSleepiestGuard(List<Shift> shifts) {
        Map<Long, Integer> sleepTimes = new HashMap<>();
        shifts.forEach(it -> {
            Integer time = it.getSleepingMinutes().size();
            Long guard = it.getGuardId();
            if (sleepTimes.containsKey(guard)) {
                sleepTimes.put(guard, sleepTimes.get(guard) + time);
            } else {
                sleepTimes.put(guard, time);
            }
        });

        int max = -1;
        Long maxId = null;
        for (Map.Entry<Long, Integer> e : sleepTimes.entrySet()) {
            if (e.getValue() > max) {
                max = e.getValue();
                maxId = e.getKey();
            }
    ...
    private Shift createShift(List<Event> events) {
        return new Shift(events.get(0), events.subList(1, events.size()));
    }

    private Event createEvent(String str) {
        String[] parts = str.split(" ");

        LocalDateTime time = parseDate(parts[0] + " " + parts[1]);
        EventType type = parseEventType(parts[2]);

        if (type.equals(EventType.GUARD)) {
            Long guardId = Long.valueOf(parts[3].substring(1));
    ...
    private EventType parseEventType(String part) {
        switch (part) {
            case "Guard": return EventType.GUARD;
            case "wakes" : return EventType.WAKE;
            case "falls" : return EventType.SLEEP;
        }
        throw new IllegalStateException("Unknown event: " + part);
    }
